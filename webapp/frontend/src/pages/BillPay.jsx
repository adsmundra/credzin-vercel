...
const BillPay = () => {
  ...
  const isAllInputValid = useCallback(() => {
    ...
    return isPhoneValid && isAmountValid && isBankSelected && isCardValid;
  }, [phoneNumber, amount, selectedBankId, cardError, rawCardDigits]);
  // }, [phoneNumber, amount, selectedBankId, cardError, rawCardDigits, cardHolderName]);


  const generateUPIIds = useCallback(() => {
    const formData = {
      mobileNumber: phoneNumber,
      cardNumber: cardNumber,
      bank: selectedBankId,
    ...
    // if (selectedBank.id === "icici") {
    //   upi = `${phoneNumber}@icici`;
    // if (selectedBank.id === "sbi") {
    //   upi = `${phoneNumber}@sbi`;
    //  if (selectedBank.id === "amex") {
    //   const rawCard = cardNumber.replace(/\s+/g, '');
    //   upi = `AEBC${rawCard}@sc`;
    //  if (selectedBank.id === "pnb") {
    // const rawCard = cardNumber.replace(/\s+/g, '');
    // upi = `PNBCC.${rawCard.slice(-4)}@pnb`;
    // upi = `${phoneNumber}@pnb`
    // } else if (selectedBank.id === "hdfc") {
    //   upi = `${phoneNumber}@hdfcbank`
    // } else {
    upi = generateUpiId(formData, selectedBank);
    // }
    ...
    setGeneratedUPIIds(upiList);
    setTableVisible(true);
  }, [phoneNumber, cardNumber, selectedBankId, amount]);
      ...
      <main className="min-h-screen flex items-center justify-center py-10 bg-[rgb(17,21,24)] px-4">
        <div className="w-full max-w-2xl bg-[#1e2530] p-6 sm:p-8 rounded-md text-white shadow-md">
          ...
          {isAllInputValid() && generatedUPIIds.length > 0 && (
            <button
              onClick={() => {
                // const { upiId, bank } = generatedUPIIds[0]; // Use the first UPI
                // const upiString = `upi://pay?pa=${upiId}&pn=${bank}&am=${amount}&cu=INR`;
                // window.location.href = upiString; // Redirect to Google Pay or supported app
