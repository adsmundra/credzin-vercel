import { Bank, FormData, ValidationErrors, CardValidationResponse } from './types';
...
export const validateForm = (data: FormData): ValidationErrors => {
  ...
  if (!data.cardNumber.replace(/\D/g, '').match(/^\d{16}$/)) {
...
export const generateUpiId = (data: FormData, bank: Bank): string => {
  const cleanCardNumber = data.cardNumber.replace(/\D/g, '');
  const last4 = cleanCardNumber.slice(-4);
  
  let upiId = bank.upiFormat
...
export const validateCard = async (cardNumber: string): Promise<CardValidationResponse> => {
...
export const findBankByIssuer = (issuer: string): Bank | undefined => {
  return BANKS.find(bank => bank.issuerPattern.test(issuer));
};

export const getCardSchemeIcon = (scheme: string): string => {
