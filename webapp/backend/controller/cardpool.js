...
exports.getGroupWithMembersAndCards=async(req, res) => {
  try {
    ...
    const enhancedGroupMembers = await Promise.all(groupMembers.map(async (member) => {
      if (member.user_id && member.user_id.CardAdded && member.user_id.CardAdded.length > 0) {
        // Get all generic card IDs from active user cards only
        const genericCardIds = member.user_id.CardAdded
          .filter(userCard => userCard.user_card_status === 'ACTIVE')
        ...
        const enhancedUserCards = member.user_id.CardAdded
          .filter(userCard => userCard.user_card_status === 'ACTIVE')
          .map(userCard => ({
            ...userCard.toObject(),
